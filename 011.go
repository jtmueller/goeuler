...
func run011() {
	...
	nums := [][]byte{
		[]byte{8, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12, 50, 77, 91, 8},
		...
		[]byte{67, 26, 20, 68, 02, 62, 12, 20, 95, 63, 94, 39, 63, 8, 40, 91, 66, 49, 94, 21},
		[]byte{24, 55, 58, 05, 66, 73, 99, 26, 97, 17, 78, 78, 96, 83, 14, 88, 34, 89, 63, 72},
		[]byte{21, 36, 23, 9, 75, 00, 76, 44, 20, 45, 35, 14, 00, 61, 33, 97, 34, 31, 33, 95},
		[]byte{78, 17, 53, 28, 22, 75, 31, 67, 15, 94, 03, 80, 04, 62, 16, 14, 9, 53, 56, 92},
		...
		[]byte{04, 52, 8, 83, 97, 35, 99, 16, 07, 97, 57, 32, 16, 26, 26, 79, 33, 27, 98, 66},
		[]byte{88, 36, 68, 87, 57, 62, 20, 72, 03, 46, 33, 67, 46, 55, 12, 32, 63, 93, 53, 69},
		[]byte{04, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18, 8, 46, 29, 32, 40, 62, 76, 36},
	...
	for i := 0; i < 20; i++ {
		for j := 0; j < 16; j++ {
			// horiz products
			prod = get(i, j) * get(i, j+1) * get(i, j+2) * get(i, j+3)
			if prod > maxProd {
				maxProd = prod
			}
			// vert products
			prod = get(j, i) * get(j+1, i) * get(j+2, i) * get(j+3, i)
			if prod > maxProd {
				maxProd = prod
			}
		}
	...
	for i := 0; i < 16; i++ {
		for j := 0; j < 16; j++ {
			prod = get(i, j) * get(i+1, j+1) * get(i+2, j+2) * get(i+3, j+3)
			if prod > maxProd {
				maxProd = prod
			}
		}
	...
	fmt.Printf("The greatest product of four adjacent numbers is: %d.", maxProd)
}
