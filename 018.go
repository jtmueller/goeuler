...
func maxSum(nums [][]int, r, c int) int {
	x := nums[r][c]
	lf := x + nums[r+1][c]
	rt := x + nums[r+1][c+1]
	if lf > rt {
...
func run018() {
	...
	for r := len(nums) - 2; r >= 0; r-- {
		for c, ln := 0, len(nums[r]); c < ln; c++ {
			nums[r][c] = maxSum(nums, r, c)
		}
	}

	sum := nums[0][0]

	fmt.Printf("Maximum sum of paths through the triangle is: %d", sum)
}
